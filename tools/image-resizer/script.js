...
function processFiles(files) {
    ...
    imageFiles.forEach(file => {
        if (!uploadedFiles.find(f => f.name === file.name && f.size === file.size)) {
            // Create a persistent object URL for preview and reuse
...
async function resizeImages() {
    ...
    // Revoke previous result URLs before clearing
    resizedImages.forEach(result => {
        if (result.url) URL.revokeObjectURL(result.url);
    });
    resizedImages = [];
...
function displayResults() {
    ...
    resizedImages.forEach((result, index) => {
        ...
        resultItem.innerHTML = `
            <img src="${result.url}" alt="Resized" class="result-preview">
...
function previewResizedImage(index) {
    ...
    modal.innerHTML = `
        <div style="max-width: 90%; max-height: 90%; position: relative;">
            <img src="${result.url}" style="max-width: 100%; max-height: 100%; object-fit: contain;">
            <button onclick="this.parentElement.parentElement.remove()" style="
                position: absolute;
                top: -40px;
                right: 0;
